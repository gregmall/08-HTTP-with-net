...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    if(request.path === '/' && request.method === 'GET'){
      ...
      socket.end(createResponse({ body: '<html><body><h1>red</h1></body></html>', status: '200 ok', contentType: 'text/plain' }));

    } else if(request.path === '/green' && request.method === 'GET'){
      socket.end(createResponse({ body: '<html><body><h1>green</h1></body></html>', status: '200 ok', contentType: 'text/plain' }));

    }
    else if(request.path === '/blue' && request.method === 'GET'){
      socket.end(createResponse({ body: '<html><body><h1>blue</h1></body></html>', status: '200 ok', contentType: 'text/plain' }));
